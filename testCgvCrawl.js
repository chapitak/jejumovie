...
async function testCgvCrawl() {
    ...
    console.log("Starting CGV multi-branch and multi-day crawl test...");
    ...
    try {
        ...
        for (const branch of branches) {
            const filePath = path.join(__dirname, 'data', `cgv_${branch.name}.json`);
            if (fs.existsSync(filePath)) {
                console.log(`Test Passed: cgv_${branch.name}.json file was created.`);
                const fileContent = fs.readFileSync(filePath, 'utf8');
                const jsonData = JSON.parse(fileContent);

                if (typeof jsonData === 'object' && Object.keys(jsonData).length > 0) {
                    console.log(`Test Passed: cgv_${branch.name}.json contains valid object data.`);

                    for (const date of dates) {
                        if (jsonData[date] && Array.isArray(jsonData[date])) {
                            console.log(`Test Passed: CGV ${branch.name} on ${date} has an array of schedules.`);
                            ...
                            console.error(`Test Failed: CGV ${branch.name} on ${date} is missing or not an array.`);
                            process.exit(1);
                        }
                    }
                } else {
                    console.error(`Test Failed: cgv_${branch.name}.json is empty or invalid object.`);
                    process.exit(1);
                }
            } else {
                console.error(`Test Failed: cgv_${branch.name}.json file was not created.`);
                process.exit(1);
            }
        }
    } catch (error) {
    ...
    console.log("CGV multi-branch and multi-day crawl test completed.");
}
